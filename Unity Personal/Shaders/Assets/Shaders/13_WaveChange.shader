...
Shader "Steve/tut/13 - Wave Change"{
	Properties {
		...
		_MidThreshold ("Mid Height Threshold", Range(2.0,4.0)) = 2
		_HighThreshold ("High Height Threshold", Range(2.0,4.0)) = 4
	...
	SubShader {
		Pass {
			...
			{
             return  frac(sin( dot(myVector ,float3(12.9898,78.233,45.5432) * 4477.5522)) );
         	}

			float random( float3 p ) // Version 2
			{
			   // e^pi (Gelfond's constant)
			   // 2^sqrt(2) (Gelfond–Schneider constant)
			   float3 r = float3( 23.14069263277926, 2.665144142690225, 43.2342348939842 );
			...
			fixed4 frag(vertexOutput i) : COLOR
			{
				...
				if (_isBending)
				{
					if (i.objectDist > _MidThreshold + _SinTime.w * lerp (-.4, .4, noise(float3(i.worldPos))))//_SinTime.w / (i.objectDist) * _MorphSpeed)
					{
						tex = tex2D(_MainTex, i.tex.xy * _MainTex_ST.xy + _MainTex_ST.zw);
						texN = tex2D(_BumpMap, i.tex.xy * _BumpMap_ST.xy + _BumpMap_ST.zw);

					} 
					else if (i.objectDist > _HighThreshold + _SinTime.w *  lerp (-.3, .3, noise(float3(i.worldPos))))//_SinTime.w / (i.objectDist) * _MorphSpeed)
					{
